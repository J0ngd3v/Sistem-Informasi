...
<div class="container">
          ...
          <form action="" method="POST">
            <div class="row justify-content-center">
                <div class="col-md-12">
                  ...
                  <button name="cari" class="btn btn-primary mt-2 " style="margin-left:90px;">Cari Pelajaran</button>
                </div>
            </div>
          </form>
      </div>
  ...
  <?php
    ...
    if(isset($_POST['cari'])){
      $query = "SELECT*FROM jadwal_pelajaran WHERE hari='".$_POST['hari']."'";
      $kirim = mysqli_query($config , $query);
      while($pelajaran = mysqli_fetch_assoc($kirim)){
  ?>
  <div class="row justify-content-center">
        <div class="col-md-4">
        <div class="card mt-2" style="opacity:0.7;">
                   <tr><td><?= $pelajaran['nama'];?></td><td></td></tr>
                </div>
        </div>
  </div>
  <?php } ?>
